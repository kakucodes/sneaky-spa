...
import { SHOP_SHEETS_CONFIG } from "../config";

const StoreItemSchema = z.object({
  image: z.string().min(1, "Image URL is required"),
  name: z.string().min(1, "Name is required"),
  basedOn: z.string().min(1, "Based on NFT reference is required"),
...
export const getStoreItems = async (): Promise<
  | { success: true; items: StoreItem[] }
  | { success: false; errors: ValidationError[] }
  | "Failed to load items"
> => {
  try {
    let storeItems = await loadSheetData(SHOP_SHEETS_CONFIG, StoreItemSchema);
